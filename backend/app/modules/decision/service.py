...
class DecisionService:
    ...
    def create_decision(
        self, decision_data: DecisionCreate, created_by: int
    ) -> DecisionResponse:
    ...
    def update_decision(
        self, decision_id: int, decision_data: DecisionUpdate
    ) -> DecisionResponse:
        ...
        if "status" in update_data:
            ...
            if (
                ...
                and decision.status != DecisionStatus.APPROVED
            ):
    ...
    def submit_vote(
        self, decision_id: int, user_id: int, vote: str, comment: str = None
    ) -> None:
        """Karara oy verir."""
        # Karar kontrolü
        decision = self.repository.get_by_id(self.db, decision_id)
        ...
        if vote not in valid_votes:
            raise ValidationError(
                f"Geçersiz oy değeri. Geçerli değerler: {', '.join(valid_votes)}"
            )
