...
class DecisionVoteRepository:
    ...
    def get_by_decision_and_user(
        db: Session, decision_id: int, user_id: int
    ) -> Optional[DecisionVote]:
        """Karar ve kullanıcı ID'sine göre oy getirir."""
        return (
            db.query(DecisionVote)
            .filter(
                DecisionVote.decision_id == decision_id, DecisionVote.user_id == user_id
            )
            .first()
        )
    ...
    def get_by_decision(db: Session, decision_id: int) -> List[DecisionVote]:
        ...
        return (
            db.query(DecisionVote).filter(DecisionVote.decision_id == decision_id).all()
        )
